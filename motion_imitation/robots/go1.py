...
from motion_imitation.robots import go1_robot_velocity_estimator
...
# 3 степени свободы на ногу.
DOFS_PER_LEG = 3
# Создаем массив из 3-х угловых положений для каждой ноги.
JOINT_OFFSETS = np.array(
    [HIP_JOINT_OFFSET, UPPER_LEG_JOINT_OFFSET, KNEE_JOINT_OFFSET] * 4)
PI = math.pi

# Задаем максимальное изменение угла за шаг.
...
# Устанавливаем максимальный крутящий момент. Пик, после которого показатель не растет.
...
URDF_FILENAME = os.path.join(parentdir, "motion_imitation/utilities/go1/go1.urdf")
...
class Go1(minitaur.Minitaur):
  ...
  def __init__(
    ...
    if velocity_source is VelocitySource.IMU_FOOT_CONTACT:
     self._velocity_estimator = go1_robot_velocity_estimator.VelocityEstimator(
         robot=self,
  ...
  def ApplyAction(self, motor_commands, motor_control_mode=None):
    ...
    super(Go1, self).ApplyAction(motor_commands, motor_control_mode)
